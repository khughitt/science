...
For pipelines that produce a processed analysis table consumed by models or statistics, add a dedicated data-QA step — one script + one rule that reads the processed table and writes a `qa_report.md`, splitting flags into build-fatal **structural** violations vs surfaced-not-fatal **distribution** flags, with config-driven bounds as the single source of truth. Complements the per-stage assertions in the `computational-analysis` aspect; does not replace them. See [`docs/conventions/pipeline-qa-checkpoints.md`](conventions/pipeline-qa-checkpoints.md). To audit and refactor existing pipelines across all three axes (QA, organization, portability), see [`docs/process/pipeline-audit-and-refactor.md`](process/pipeline-audit-and-refactor.md). Workflow-run / derived-dataset manifests should date themselves from run identity, not regeneration wall-clock — see [`docs/conventions/reproducible-manifest-dates.md`](conventions/reproducible-manifest-dates.md).
