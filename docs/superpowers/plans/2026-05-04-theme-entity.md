...
**Tech Stack:** Python 3.13, Pydantic models in `science-model`, Click CLI helpers in `science-tool`, rdflib materialization, pytest, template renderer in `science_model.templates`.
...
- Modify `science-tool/src/science_tool/graph/entity_registry.py`: register `theme` as an epistemic core kind backed by `ThemeEntity`.
- Modify `science-tool/src/science_tool/entities.py`: add source-authored path, default status, and allowed statuses for `theme`.
...
- Modify `science-tool/tests/test_kind_class.py`: registry classification coverage for `theme`.
- Modify `science-tool/tests/test_load_project_sources_unified.py`: loader produces a `ThemeEntity`.
- Modify `science-tool/tests/test_entities.py`: CLI source-authoring path/template coverage for `theme`.
- Modify `science-tool/tests/test_graph_materialize.py`: graph build emits `SCI.Theme` and normal related edges for theme entities.
...
- Modify: `science-tool/tests/test_kind_class.py`
- Modify: `science-tool/tests/test_load_project_sources_unified.py`

- [ ] **Step 1: Write failing registry classification test**

In `science-tool/tests/test_kind_class.py`, add this assertion to `test_kind_class_lookup_returns_classification`:
...
```bash
cd science-tool && uv run --frozen pytest tests/test_kind_class.py -q -k "kind_class_lookup_returns_classification"
```
...
In `science-tool/tests/test_load_project_sources_unified.py`, update the import block from `science_model.entities` to include `ThemeEntity`:
...
```bash
cd science-tool && uv run --frozen pytest tests/test_load_project_sources_unified.py -q -k "typed_theme_entity"
```
...
In `science-tool/src/science_tool/graph/entity_registry.py`, add `ThemeEntity` to the import list:
...
```bash
cd science-tool && uv run --frozen pytest tests/test_kind_class.py tests/test_load_project_sources_unified.py -q -k "theme or kind_class_lookup_returns_classification or classifies_every_kind"
```
...
```bash
git add science-tool/src/science_tool/graph/entity_registry.py science-tool/tests/test_kind_class.py science-tool/tests/test_load_project_sources_unified.py
...
In `science-tool/src/science_tool/entities.py`, add this entry to `_BUILTIN_MARKDOWN_POLICIES`:
...
In `science-tool/tests/test_entities.py`, update `test_builtin_path_policy_maps_core_kinds` with:
...
cd ../science-tool && uv run --frozen pytest tests/test_entities.py -q -k "theme or builtin_path_policy or template_driven_create_entity"
```
...
```bash
git add science-model/src/science_model/templates.py science-model/src/science_model/templates/theme.md templates/theme.md science-model/tests/test_templates.py science-tool/src/science_tool/entities.py science-tool/tests/test_entities.py
...
```bash
cd science-tool && uv run --frozen pytest tests/test_graph_materialize.py -q -k "theme_node"
```
...
```bash
cd science-tool && uv run --frozen pytest tests/test_graph_materialize.py tests/test_load_project_sources_unified.py tests/test_health.py -q -k "theme or unresolved or materialize_graph"
```
...
Append this test to `science-tool/tests/test_entities_cli.py`:
...
```bash
cd science-tool && uv run --frozen pytest tests/test_entities_cli.py -q -k "theme_cli_round_trips"
```
...
```bash
git add science-tool/tests/test_entities_cli.py
...
```bash
uv run --frozen ruff format science-model science-tool
```
...
```bash
uv run --frozen ruff check science-model science-tool
```
...
cd ../science-tool && uv run --frozen pytest tests/test_kind_class.py tests/test_load_project_sources_unified.py tests/test_entities.py tests/test_entities_cli.py tests/test_graph_materialize.py -q
```
...
uv run --project /home/keith/d/science/science-tool --frozen science-tool entity create theme "Transportability Across Cancer Types" --id theme:transportability-across-cancer-types --no-hints
uv run --project /home/keith/d/science/science-tool --frozen science-tool refs check
uv run --project /home/keith/d/science/science-tool --frozen science-tool graph build
...
```bash
git add science-model science-tool
...
- Scope check: The plan deliberately avoids adding theme-specific predicates, migration tooling, or a bespoke `science-tool theme` command. Generic `science-tool entity create theme ...` is the v1 CLI surface.
