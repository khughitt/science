...
3. Copy every other owned file to
   `references/<path-relative-to-router-parent>`.
4. Rewrite links within the owned subtree to their new relative package paths.
5. Rewrite a link leaving the owned subtree to “Load the
   `<owning-generated-name>` skill” and record that dependency.
...
depend on the development platform.
