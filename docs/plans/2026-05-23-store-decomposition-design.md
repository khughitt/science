...
**Status:** Approved design (2026-05-23). Next: implementation plan via writing-plans, then subagent-driven-development.

**Goal:** Break the 4,773-line `science/src/science_tool/graph/store.py` monolith into a `store/` package of focused, single-responsibility submodules — a pure, behavior-preserving code-move — before starting evidence-aggregation Phase 2.
...
**Non-goals:** No behavior change. No signature change. No constant-value change. No deduplication of code that happens to be triplicated elsewhere (e.g. `_graph_uri` also exists in `io.py` and `materialize.py` — left alone). No new feature work. No changes to `graph/__init__.py`. No `query_uncertainty` belief-unification (that is deferred to Phase 2).
...
`store.py` (module) becomes `store/` (package). Code moves into focused submodules arranged as a strict dependency DAG. `store/__init__.py` re-exports the complete current public-and-used symbol surface, so every existing importer keeps working unchanged.

```
graph/store/
  __init__.py          # re-exports the public+used surface (incl. private helpers)

  ── foundation (no intra-package deps) ──
  constants.py
  types.py
  identity.py
  dataset.py

  ── mid-layer (depend only on foundation) ──
  mutations.py
  export.py
  inquiry.py
  snapshot.py
  ...
  ── read layer (depend on evidence_signals + foundation) ──
  queries.py
  summary.py
  dot.py
  notebooks.py
...
Dependency rule: foundation → mid-layer → read layer is a one-directional DAG. No submodule imports a submodule in a layer above it. `store/__init__.py` imports from all submodules; nothing inside the package imports `store/__init__.py`. External `science_model.*`, `graph.io`, `graph.export_types`, `graph.belief`, and `graph.sources` imports stay exactly where the moved code already uses them. No submodule imports `graph.materialize` (materialize depends on store, never the reverse — this invariant must hold to avoid a cycle).
...
- Namespace re-exports from `graph.io`: `CITO_NS`, `PROJECT_NS`, `SCI_NS`, `SCHEMA_NS`, `SCIC_NS`, `BIOLINK_NS`, `DCTERMS_NS`, `REVISION_URI` (so consumers importing these "from store" keep working).
...
### identity.py
URI / canonical-id / token / relation-claim-URI helpers.
...
### dataset.py
Dataset construction / load / save / stats.
- `init_graph_file` (520), `read_graph_stats` (531), `_load_dataset` (4668), `_save_dataset` (4677), `save_graph_dataset` (4681).

### mutations.py  (largest, ~1,000 lines — kept whole; all graph writes)
...
- `set_boundary_role` (1344), `set_param_metadata` (1531), `set_treatment_outcome` belongs to inquiry (see below).
...
### inquiry.py
- `list_inquiries` (1891), `get_inquiry` (1943), `set_treatment_outcome` (2024), `render_inquiry_doc` (2057), `validate_inquiry` (2297).

### snapshot.py
- `import_snapshot` (2713), `stamp_revision` (2750).

### validation.py
- `query_predicates` (2989), `validate_graph` (2993), `diff_graph_inputs` (3111).

### evidence_signals.py
Evidence/proposition data-gathering shared by `queries.py` and `summary.py`.
...
### summary.py
Summary/rollup builders, dashboards, coverage/gaps/uncertainty.
...
### dot.py
- `build_graph_dot` (4487), `_has_cycle` (4533).

### notebooks.py
Viz-notebook scaffolding.
- `_uv_lock` (4727), `_NOTEBOOKS_PYPROJECT` (4735), `_copy_viz_notebook` (4750).

## The `store/__init__.py` re-export contract

`store/__init__.py` MUST make every symbol below importable as `from science_tool.graph.store import <name>`, so all ~15 source importers and 28 test files keep working with **zero edits**. Private (underscore) names are excluded from `import *`, so they are imported **explicitly** by name — the package must not rely on `__all__` to surface them.

**Public surface (verified across `science/src` + `science/tests`):**
`DEFAULT_GRAPH_PATH`, `GRAPH_LAYERS`, `CURIE_PREFIXES`, `PROJECT_ENTITY_PREFIXES`, `PREDICATE_REGISTRY`, `GRAPH_EXPORT_EDGE_METADATA_PREDICATES`, `INITIAL_GRAPH_TEMPLATE`, `CITO_NS`, `PROJECT_NS`, `SCI_NS`, `SCHEMA_NS`, `SCIC_NS`, `PropositionEvidenceLine`, `PropositionInteractionTerm`, `PropositionPhase1Metadata`, `PropositionEvidenceSemantics`, `FalsificationRecord`, `add_article`, `add_assumption`, `add_concept`, `add_discussion`, `add_edge`, `add_evidence_edge`, `add_falsification`, `add_finding`, `add_hypothesis`, `add_inquiry`, `add_inquiry_edge`, `add_inquiry_node`, `add_interpretation`, `add_mechanism`, `add_observation`, `add_paper_entity`, `add_proposition`, `add_question`, `add_story`, `add_transformation`, `set_boundary_role`, `set_treatment_outcome`, `migrate_addresses_direction`, `export_graph_payload`, `get_inquiry`, `list_inquiries`, `validate_inquiry`, `import_snapshot`, `init_graph_file`, `stamp_revision`, `read_graph_stats`, `query_claims`, `query_coverage`, `query_dashboard_summary`, `query_evidence`, `query_gaps`, `query_inquiry_summary`, `query_neighborhood`, `query_neighborhood_summary`, `query_predicates`, `query_project_summary`, `query_question_summary`, `query_uncertainty`, `validate_graph`, `diff_graph_inputs`, `build_graph_dot`, `canonical_id_from_entity_uri`, `shorten_uri`, `save_graph_dataset`.

**Private helpers other modules import (must be explicitly re-exported):**
`_resolve_term`, `_evidence_targets_for_uri`, `_graph_uri`, `_load_dataset`, `_save_dataset`, `_collect_evidence_signals`, `_edge_claims`, `_source_strings`, `_claim_summary_data`, `_load_proposition_bridge_hypotheses`, `_load_proposition_evidence_semantics`, `_load_proposition_falsifications`, `_load_proposition_interaction_terms`, `_load_proposition_phase1_metadata`, `_load_proposition_pre_registrations`, `_slug`.

(This contract is a floor, not a ceiling: any symbol any current importer references must remain importable. Pytest collection across all 28 store-touching test files is the executable check — a missing re-export fails at import time.)

## Data flow / error handling

- **Data flow** is the dependency DAG above; no runtime control flow changes.
- **Error handling** is unchanged. `click.echo` warnings, raised exceptions, and `strict` flags move verbatim with their functions.

## Testing strategy

Behavior-preserving move ⇒ **no test edits**. That zero-churn is itself the proof of correctness:
- `cd science && uv run pytest` — green, unchanged.
- `cd science/model && uv run pytest` — green, unchanged (sanity; model package shouldn't be affected).
- Import smoke check for the private re-exports, e.g.:
  `python -c "from science_tool.graph.store import _collect_evidence_signals, _load_dataset, _resolve_term, _graph_uri, _claim_summary_data, _edge_claims; print('ok')"`
- If any test imports a symbol the new `__init__` failed to re-export, pytest collection fails immediately — caught before any logic runs.
...
- Incremental, each commit green: foundation submodules first (`constants`, `types`, `identity`, `dataset`), then mid-layer (`mutations`, `export`, `inquiry`, `snapshot`, `validation`, `evidence_signals`), then read layer (`queries`, `summary`, `dot`, `notebooks`), finalizing `store/__init__.py` and removing the old monolith.
...
1. `store.py` is gone; `store/` package exists with the 14 submodules above plus `__init__.py`.
...
3. Both test suites pass with **zero test edits**.
4. No source importer outside `store/` changed (verified by `git diff --stat` touching only `graph/store*`).
5. The dependency DAG holds (no intra-package upward imports; no import of `graph.materialize`).
