...
Status: approved; implementation underway — foundation substrate + Pillar C sub-phases C1/C2/C3/C4a/C4b merged + Pillar A (A1 + A2) merged and complete; Pillar D1, B1, B-migration, B2, and E (Reactome ingestion) merged locally; C4c, D2, and non-tabular reference modeling remain open. Spawns focused per-area docs.
...
|---|---|---|---|---|
| 1 | Identity, reference genomes & id mapping (C) | — | canonical assembly + gene/protein/variant crosswalks; pinned-vs-service policy | design ✓; **impl: C1 (assembly), C2 (gene), C3 (protein), C4a (variant identity), and C4b (liftover/compatibility) merged; C4c pending** |
...
metadata merge, and `belief-logodds-v3`). Pillar A is complete; C (C1–C3 merged, C4a/C4b merged, C4c
