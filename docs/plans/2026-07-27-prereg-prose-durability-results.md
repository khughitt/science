...
validate` call and the `[ ! -s "$out" ]` fatal check), in the second. Every
...
`"prereg.prose-path-nondurable" not in gated` alongside the four durability
...
**Not clean — a real, narrow diff, not the predicted no-op.**
`uv run --frozen pytest tests/validate -q -m snapshot` reported 1 failed, 1
passed. `json_default.json` matched exactly, as predicted. `text_default.txt`
did not: it diverged on exactly one line —
...
— with every other line, including the full list of findings against the
`_combined` fixture, byte-identical. Cause: `_combined` has no
`entities/pre-registrations/` directory, so `prereg.prose-path-nondurable`
correctly produces zero findings there, exactly as the plan predicted for
*content*. But the "Checks: N included" line counts total *registered* rules,
not rules that fired — and registering the new rule moved that count from 68
to 69 everywhere, including this fixture. `git log --oneline -- tests/validate/
snapshots/text_default.txt` shows the snapshot was last regenerated by an
unrelated commit (`b59a38d0`) that predates both rule-shipping commits
(`80d378e8`, `7cbc7e6b`); neither of those commits touched it. The snapshot is
stale, not the rule.
...
touched. `tests/validate/snapshots/text_default.txt` needs a one-line update
(68 → 69) before this branch merges.
