...
```json
{
  "data": {
    "hits": [
      ...
      {
        "id": "438b6fcb-c193-49bb-8fc8-796ab701f0eb",
        ...
        "cases": [
          {
            ...
            "samples": [
              {
                "submitter_id": "MMRF_0003_1_BM_CD138pos",
                "sample_type": "Primary Blood Derived Cancer - Bone Marrow"
              }
            ]
          }
...
def normalize_file_hit(hit: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(cases, list) or len(cases) != 1:
        raise ValueError(f"{file_id}: expected exactly one linked case, got {0 if not isinstance(cases, list) else len(cases)}")
    case = cases[0]
    samples = case.get("samples")
    if not isinstance(samples, list) or len(samples) != 1:
        raise ValueError(f"{file_id}: expected exactly one linked sample, got {0 if not isinstance(samples, list) else len(samples)}")
    sample = samples[0]
    return {
...
def discover_endpoint_fields(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    usable_progression_count = sum(1 for row in flattened if _usable_progression_outcome(row))
    if usable_progression_count > 0:
    ...
    return {
        ...
        "usable_progression_outcome_count": usable_progression_count,
...
def write_dry_run(*, output_dir: Path, client: GdcClient) -> dict[str, Any]:
    ...
    report = {
        ...
        "usable_progression_outcome_count": endpoint["usable_progression_outcome_count"],
...
def download_expression_files(*, output_dir: Path, manifest_rows: list[dict[str, Any]]) -> None:
...
def _download(url: str, output_path: Path) -> None:
    ...
    with urllib.request.urlopen(url, timeout=HTTP_TIMEOUT_SECONDS) as response, tmp_path.open("wb") as fh:
...
def _flatten_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(diagnoses, list) or len(diagnoses) != 1 or not isinstance(diagnoses[0], dict):
        raise ValueError(f"{case.get('submitter_id')}: expected exactly one diagnosis record")
    out.update(diagnoses[0])
    demographic = case.get("demographic")
    if isinstance(demographic, dict):
        out.update({key: demographic.get(key) for key in SURVIVAL_FIELD_NAMES})
    return out


def _usable_progression_outcome(row: dict[str, Any]) -> bool:
    status = str(row.get("progression_or_recurrence") or "").strip().lower()
    if status in {"yes", "true", "1", "progression", "recurrence"}:
        return row.get("days_to_recurrence") not in (None, "")
    if status in {"no", "false", "0", "no recurrence"}:
        return row.get("days_to_last_follow_up") not in (None, "")
    return False
...
def main() -> None:
    ...
    if args.download_expression:
        ...
        download_expression_files(output_dir=output_dir, manifest_rows=manifest)
...
def build_package(*, output_dir: Path, measure: str = DEFAULT_MEASURE, split_salt: str = DEFAULT_SPLIT_SALT) -> dict[str, Any]:
    ...
    validate_nonempty_splits(split_df)
...
def build_patient_splits(case_submitter_ids: list[str], *, split_salt: str) -> pd.DataFrame:
    if len(case_submitter_ids) < 3:
        raise ValueError("held-out-patient splits require at least three patients for nonempty train/validation/test splits")
    ranked: list[tuple[str, str]] = []
    for case_id in case_submitter_ids:
        digest = hashlib.sha256(f"{case_id}{split_salt}".encode("utf-8")).hexdigest()
        ranked.append((digest, case_id))
    ranked.sort()
    ...
    if train_count < 1:
        raise ValueError("held-out-patient splits require at least one training patient")

    rows: list[dict[str, str]] = []
    for index, (digest, case_id) in enumerate(ranked):
...
def validate_nonempty_splits(split_df: pd.DataFrame) -> None:
    observed = set(split_df["split"])
    required = {"train", "validation", "test"}
    missing = sorted(required - observed)
    if missing:
        raise ValueError(f"held-out-patient splits must be nonempty; missing {missing}")
...
def _build_outcomes(cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for case in cases:
        ...
        if normalized_status in {"unknown", "not reported", "not applicable", ""}:
            continue
        if normalized_status in {"yes", "true", "1", "progression", "recurrence"}:
            days = diagnosis.get("days_to_recurrence")
            event_observed = True
            source_days_field = "days_to_recurrence"
        elif normalized_status in {"no", "false", "0", "no recurrence"}:
            days = diagnosis.get("days_to_last_follow_up")
            event_observed = False
            source_days_field = "days_to_last_follow_up"
        else:
            raise ValueError(f"{case.get('submitter_id')}: unsupported progression_or_recurrence value {status!r}")
        if days in (None, ""):
            raise ValueError(f"{case.get('submitter_id')}: missing time-to-event/censoring days for progression outcome")
        rows.append(
            {
                ...
                "source_days_field": source_days_field,
...
RESOURCE_FILES = {
    ...
    "cases": ("manifest/cases.json", "json", "application/json"),
    ...
    "build_summary": ("reports/build-summary.json", "json", "application/json"),
}
...
def build_datapackage_doc(data_dir: Path, *, split_salt: str, gdc_data_release: str) -> dict[str, Any]:
    ...
    return {
        ...
        "split": {
            ...
            "split_salt": split_salt,
            "allocation": "ranked hash order with nonempty validation/test minima",
        },
...
def resolve_data_dir(raw: Path | None) -> Path:
    if raw is not None:
...
the validation gates. The recipe uses GDC metadata as the source of truth. It
does not emit AWS S3 URIs until the bucket/key convention is separately
verified.
...
- GDC-only manifest metadata until AWS bucket/key conventions are verified:
  Task 2 manifest rows and Task 5 README wording.
