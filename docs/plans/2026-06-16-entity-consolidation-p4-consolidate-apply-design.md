...
- Mints a live `synthesis` entity at its canonical home `entities/synthesis/`
  (the `synthesis` kind's `home`), via the existing entity-creation path, with
  frontmatter:
  - `type: synthesis`
  ...
  - `consolidates: [member-id, …]` prefilled
...
- Re-reads the digest entity on **live state**; fails loud if the digest is
  missing, is not `report_kind: cluster-digest`, or has an empty `consolidates:`.
...
- **`archive.py`** — additive only:
  ...
  - Extract the per-row move-first → `append_row` → rollback loop currently
    ...
    project_root, rows, *, now)` primitive. `archive_entities` delegates to it;
    `consolidate.apply_consolidation` builds member-scoped rows (with
    `consolidated_into`/`digest_insight`) and calls the same primitive. DRY,
    composition over duplication, identical durability/rollback discipline.
...
On failure at step 3 or 4: move the file back (if moved) and restore the
snapshotted original bytes — leaving the member exactly as it was. This mirrors
the existing `archive_entities` move-then-append rollback, extended to also
revert the frontmatter rewrite. `digest_insight` is the member's title
...
- **`CORE_PROFILE` (`model/src/science_model/profiles/core.py`):** add `archived`
  to the **authored-epistemic markdown kinds** that are realistic consolidation
  members: `hypothesis`, `question`, `proposition`, `observation`, `finding`,
...
## 7. Reference resolution (no new wiring)

- `consolidates: [id, …]` lives on the **live digest**, points at the **archived**
  members (live→archived). These resolve through P3's existing mechanism:
  `load_project_sources` folds `load_archive_index(...).resolvable_ids()` into
  `manual_aliases`, and `check_cross_references` unions the same set into
  `all_ids`. So `consolidates` targets resolve as "known but not live" without
  rehydrating the archived files. **Already works; P4 adds a guard test for the
  consolidate-produced shape.**
- `consolidated_into: <digest-id>` (archived member → live digest) and
  `superseded_by:` resolve through **normal** live resolution — their targets are
  live. No special handling.
...
- Digest must exist, be `report_kind: cluster-digest`, and have a non-empty
  `consolidates:` list (apply step).
...
- **scaffold:** mints a digest with correct frontmatter (`type`, `report_kind:
  cluster-digest`, `consolidates`), at the synthesis home; validates members;
...
- **fail-loud:** member already archived; member is the digest; empty
  `consolidates`; digest not `cluster-digest`; member kind closed-vocab lacking
  `archived` (local + non-epistemic core).
- **atomic rollback:** simulate an append failure mid-apply → member file is back
  at its original path with original bytes; index unchanged for that member.
- **resolution guard:** after a real consolidate, `science validate` and the
  graph build resolve `consolidates` → archived members (no nonexistent-reference
  error, no archived file forced live); `consolidated_into` resolves to the live
  digest.
- **discussions check:** `cluster-digest` accepted as a valid `report_kind`.
- **status vocab:** the listed authored-epistemic kinds now accept `archived`;
...
`archived` goes on the authored-epistemic kinds (§6); closed-vocab kinds fail loud
