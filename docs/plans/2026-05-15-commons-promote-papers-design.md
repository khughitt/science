...
- **Mid-step 5 (commit landed, some tags failed)** — handled by `_rollback_step5` (§5.2): delete every tag that was created in this step, then `git reset --hard HEAD~1`. Commons returns to a clean pre-step-4 state; no project files were touched. Result: `status: failed`, `failure_stage: "write_commons"`. Tag preflight at step 5.1 makes this path extremely unlikely (the only remaining triggers are physical IO faults), but the recovery is wired so a partial-tags state cannot persist.
