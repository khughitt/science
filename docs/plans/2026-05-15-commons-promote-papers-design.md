...
- `_rollback_step5(commons_root: Path, tags_attempted: list[str]) -> None` — deletes any created tags via `git tag -d`, then `git reset --hard HEAD~1`. Used by the mid-step-5 recovery path (§6.3 step 5).
...
5. **Commit + tag** — three sub-steps:
   ...
   2. **Commit.** `git -C <commons> add papers/ && git commit -m "promote: N papers via op <op-id>"`.
   ...
   **Mid-step-5 failure recovery.** If any individual `git tag` fails after the commit has landed (extremely unlikely given the preflight, but possible — e.g., filesystem error, signal), the recovery is automatic: delete every `paper/<bibkey>/<semver>` tag that was created in this step, then `git -C <commons> reset --hard HEAD~1` to revert the just-made commit. The commons working tree returns to its pre-step-4 state (clean, per the §6.3 step 0 preflight). Then raise `PromoteWriteError(stage="write_commons", detail="tag failed after commit: <…>")` so the audit log records the rollback. No project files have been touched at this point.
