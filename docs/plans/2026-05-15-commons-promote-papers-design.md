...
### 4.1.3 Bibkey case — preserve source, match case-insensitively, canonicalize on collision

The commons adapter (`adapter.py:149`) and overlay adapter (`overlay.py:114`) derive canonical ids from filename stems case-sensitively and require frontmatter `id:` to match the stem exactly. The schema regex (`[A-Za-z]…`) permits mixed case. So promote must NOT silently lowercase ids — `Adams2025.md` with `paper:Adams2025` cannot become `adams2025.md` with `paper:adams2025` without rewriting.

The rules:

1. **Canonical bibkey case = source case.** A single-instance candidate's bibkey is the exact case from its source file (filename stem). The canonical commons file is `papers/<bibkey>.md` with that exact case; the canonical id is `paper:<bibkey>`.
2. **Dedup matching is case-insensitive.** Grouping uses `bibkey_normalized = bibkey.casefold()`; two candidates `Huh2024` and `huh2024` collide.
3. **On case collision in a multi-instance group**, the canonical case is picked deterministically: the first candidate's case, where "first" is by the `--from` argument order, then by project slug for tie-breaks. The canonical file and id use that case.
4. **Overlays must match canonical case.** When the canonical case differs from a project's source filename / id, the overlay rewrite **renames** the project file to the canonical case AND rewrites `id:` accordingly. The rename is recorded in the audit log (`overlay_rewrites[].rename: { from: "huh2024.md", to: "Huh2024.md" }`) so the user can see what changed. The preflight check (§6.3 step 0) confirms the *source* path is clean; the rename target's directory must not already contain a file at the canonical case (otherwise it's a genuine collision that promote can't resolve — abort with `PromoteInputError` naming both paths).
5. **Single-project, multi-case is not possible** within a project (since case-insensitive filesystems would already have collapsed them, and case-sensitive ones would treat them as different bibkeys); promote does not need a within-project case-normalization step.

### 4.1.4 Date coercion on overlay write

PyYAML's `safe_load` parses unquoted dates as `datetime.date` instances. JSON Schema validation expects ISO date strings (e.g. `"2026-05-15"`), and the commons / overlay validators run after `safe_load`. Promote must coerce: on **write**, every `created` / `updated` value (canonical and overlay) is serialized as a **quoted ISO-8601 date string** — `created: "2026-05-15"`, not `created: 2026-05-15`. On **read**, the parser accepts either form (PyYAML coerces both; promote calls `.isoformat()` on `date` instances before validation).

This rule applies to any date-valued frontmatter field, not just `created` / `updated`. The promote writer always quotes scalar dates.
...
class PromoteCandidate:
    bibkey: str                          # original case from source filename / id
    bibkey_normalized: str               # casefold(); used as the dedup grouping key only
...
- `_scan_project_papers(project_root: Path, project_slug: str) -> tuple[list[PromoteCandidate], list[FailedCandidate]]` — walks `<project_root>/doc/papers/*.md`, parses frontmatter, classifies each file (§6.3 step 2 rule). Per-file failure (parse error, schema-failing frontmatter, malformed `id`) produces a `FailedCandidate`; the walk continues.
...
- `_classify_paper_file_kind(frontmatter: dict) -> Literal["paper", "skip-other-kind", "skip-other-id"]` — the kind-inference rule from §6.3 step 2.
...
- `_pick_canonical_bibkey_case(candidates: list[PromoteCandidate], from_order: list[str]) -> str` — deterministic case picker (§4.1.3 rule 3).
- `_coerce_date_for_yaml(value: Any) -> str` — `datetime.date` / `datetime.datetime` / `str` → ISO-8601 string, wrapped quoted on render (§4.1.4).
- `_render_canonical(decision) -> str` — markdown writer for the commons surface file; always quotes date scalars.
- `_render_overlay(decision, project_slug) -> str` — markdown writer for the rewritten project file; always quotes date scalars; emits the canonical-case `id:`.
- `_rollback_step5(commons_root: Path, tags_attempted: list[str], canonical_paths: list[Path]) -> None` — non-destructive path-limited rollback (§6.3 step 5 recovery): delete every created tag (`git tag -d <tag>`), `git reset --soft HEAD~1` to move HEAD back without touching the working tree, then per-canonical-path `git checkout HEAD -- <path>` (or `Path.unlink(missing_ok=True)` for first-promote paths that didn't exist at HEAD). NEVER uses `reset --hard`.
- `_write_audit_log(result, commons_root) -> Path` — YAML writer for the `.migrations/` entry.
- `_normalize_bibkey_for_match(raw: str) -> str` — strips `.md`, applies `.casefold()` for dedup grouping; rejects empty / whitespace / regex-failing strings with `PromoteCandidateError`. Does NOT mutate the canonical bibkey case.
...
0. **Preflight cleanliness check** — two different cleanliness contracts, one per repo class:
   - **Commons repo (strict).** The working tree AND index must be **fully clean** — no uncommitted changes anywhere, no staged changes anywhere, no untracked files inside `papers/` or `.migrations/`. The commons store is system-managed; user-side WIP there is rare and a clean precondition lets promote commit and roll back without path-juggling guesswork. If the commons repo is dirty, abort with a `PromoteInputError` that names the offending paths AND tells the user to commit / stash before re-running.
   - **Project repos (target-scoped).** Each project's working tree must have **no uncommitted changes to in-plan target files** (`doc/papers/<bibkey>.md` for each candidate, and the rename destination if §4.1.3 rule 4 triggers a case rename). Files outside the in-plan set may be dirty without blocking — promote never touches them, and step 6's `git checkout HEAD -- <target paths>` is path-limited so unrelated dirty work is preserved.
   - **All repos** must NOT be in a merge / rebase / cherry-pick / bisect state.

   The check runs **after** discovery (step 2) and **after** the plan is built (step 3), because the in-plan target set is only known then. If any of the conditions above fails, the run aborts with `PromoteInputError` listing every offending path; nothing has been written. This is what makes the commit and rollback paths in steps 5–7 safe.
1. **Validate inputs** — every `--from` slug resolves to a non-null registered id; `<commons_root>` exists.
2. **Discover** — for each project, walk `<project_root>/doc/papers/*.md` directly via `_scan_project_papers` (NOT `build_inventory` — see §5.3). For each file: split frontmatter; classify kind per the rule below; if `overlay_of:` is present, skip (idempotency); otherwise construct a `PromoteCandidate`. A per-file parse failure or schema-failing frontmatter produces a `FailedCandidate` for that path, recorded in `DiscoveryResult.failed_candidates`; the walk continues. Group surviving candidates by `bibkey_normalized` (case-insensitive).

   **Kind classification rule** (must NOT silently drop valid papers). Since the file lives under `doc/papers/`, the default classification IS paper:
   - If frontmatter declares `kind:` or `type:` and the declared value is **`"paper"`** (explicitly) → it's a paper candidate.
   - If frontmatter declares `kind:` or `type:` with **any other value** (e.g. `kind: dataset`, `type: review-article`) → log a warning, skip the file. This is an explicit "not a paper, despite the directory."
   - If frontmatter has **no `kind:` and no `type:`** → infer from the directory: it's a paper candidate. This mirrors the existing path-based inference in `frontmatter.py:188` and matches reality (many papers in the wild omit `kind:` / `type:`).
   - As a defense-in-depth secondary check, if `id:` is present and does NOT start with `paper:`, the file is skipped with a warning (a stronger declaration than mere directory placement).

   This rule is encoded in `_classify_paper_file_kind(frontmatter) -> Literal["paper", "skip-other-kind", "skip-other-id"]` and tested directly.
...
5. **Commit + tag** — three sub-steps. All git operations are **path-limited** (explicit pathspecs after `--`); even though step 0 verified the commons repo is fully clean, defensive path-limiting protects against future preflight bugs.
   1. **Tag preflight.** For each in-plan `paper/<bibkey>/<version>` tag name, run `git -C <commons> rev-parse --verify <tag>` (silenced). If any tag already exists, abort with `PromoteWriteError(stage="write_commons")` BEFORE the commit. This is the only place we can catch this cleanly; a pre-existing tag with the same name would either get refused at `git tag` (default) or silently overwritten (`-f`), and we want neither.
   2. **Commit.** Build the explicit pathspec list `paths = [papers/<bibkey>.md for each decision]`. Run `git -C <commons> add -- <paths...>` then `git -C <commons> commit -m "promote: N papers via op <op-id>" -- <paths...>`. The trailing `-- <paths>` on `commit` is a path-limited commit: only changes to those paths are recorded, even if other files happen to be staged. (Combined with step 0's clean-commons requirement this is belt-and-braces, but the path-limited form documents intent and protects the operation if step 0 ever relaxes.)
   3. **Tag.** For each decision in deterministic (bibkey-sorted) order, `git -C <commons> tag paper/<bibkey>/<semver> <commit-sha>`. All tags point at the single batch commit.

   **Mid-step-5 failure recovery.** If any individual `git tag` fails after the commit has landed (extremely unlikely given the preflight, but possible — e.g., filesystem error, signal), the recovery uses **non-destructive, path-limited** git operations:
   1. Delete every `paper/<bibkey>/<semver>` tag that was created in this step (`git -C <commons> tag -d <tag>`).
   2. Move HEAD back one commit *without touching the working tree or index*: `git -C <commons> reset --soft HEAD~1`. (NOT `--hard`, which would clobber any work that might have raced into the commons tree between step 0 and now, even though step 0 verified clean.)
   3. For each `papers/<bibkey>.md` written in step 4, restore its pre-promote state from HEAD: `git -C <commons> checkout HEAD -- papers/<bibkey>.md`. For files that didn't exist at HEAD (a first-promote), delete the working-tree file: `Path(file).unlink(missing_ok=True)`.

   After recovery, commons returns to its pre-step-4 state (clean for promote-owned paths; anything else untouched). Raise `PromoteWriteError(stage="write_commons", detail="tag failed after commit: <…>")` so the audit log records the rollback. No project files have been touched at this point.
...
7. **Write audit log** — final step on the success path. Written to `<commons>/.migrations/<op-id>.yaml` and committed to commons via **path-limited** git operations: `git -C <commons> add -- .migrations/<op-id>.yaml` then `git -C <commons> commit -m "audit: op <op-id>" -- .migrations/<op-id>.yaml`. The trailing pathspec ensures only the audit file is committed, even if other files happen to be staged. The log carries `failed_candidates` from the plan so the user sees soft failures alongside successes.

   On the failure path (any failure at steps 0–6), the same log file is written **best-effort, uncommitted**, with `status: failed`, `failure_stage:` set, and whatever partial state is known (commons commit hash if step 5 landed, list of projects whose rewrites were rolled back, etc.). The CLI surfaces the log path in the error message. If even the best-effort log write fails (disk full, permissions), the CLI prints the would-have-been log content to stderr instead. The user can `git add -- .migrations/<op-id>.yaml && git commit -m "audit ..." -- .migrations/<op-id>.yaml` afterwards if they want the failed-run log versioned.

### 6.4 Failure handling

The step-0 preflight + path-limited git operations are what make every rollback path safe:

- **Commons-side rollback is safe** because step 0 requires the commons repo fully clean and steps 4–7 all touch only `papers/<bibkey>.md` for in-plan bibkeys and `.migrations/<op-id>.yaml`. The step-5 mid-failure recovery uses `git reset --soft` plus per-path `checkout HEAD --` rather than `reset --hard`, so even if something has raced into the commons tree between step 0 and the failure, that work isn't destroyed.
- **Project-side rollback is safe** because step 0 verifies each in-plan target file matches HEAD, so `git -C <project> checkout HEAD -- <target paths>` restores exactly the pre-promote content for those paths and leaves every other dirty file in the project untouched. The checkout is always path-limited; there is no `git -C <project> checkout HEAD` (no pathspec) anywhere in the design.
- **No commit operation in the design omits an explicit pathspec.** Every commons-side `git commit` carries `-- <path>` to defang any future preflight bug that might let staged unrelated work leak into the promote commit.
...
- **Before step 5 (commons commit)** — any failure means nothing committed. Path-limited working-tree restore: for each `papers/<bibkey>.md` already written in step 4, `git -C <commons> checkout HEAD -- papers/<bibkey>.md` if the file existed at HEAD (re-promote case) OR `Path(file).unlink(missing_ok=True)` if it did not (first-promote case). NEVER a directory-level `checkout -- papers/`, which could clobber a concurrent edit. No project files were touched. Result: `status: failed`, `failure_stage: "validate" | "discover" | "plan" | "write_commons"`.
- **Mid-step 5 (commit landed, some tags failed)** — handled by `_rollback_step5` (§5.2): delete every tag that was created, `git reset --soft HEAD~1` to move HEAD without disturbing the working tree or index, then the same per-path restore (`checkout HEAD --` or unlink) as the "before step 5" case. NEVER `reset --hard`. Commons returns to its pre-step-4 state for promote-owned paths; anything else is untouched. Result: `status: failed`, `failure_stage: "write_commons"`. Tag preflight at step 5.1 makes this path extremely unlikely (the only remaining triggers are physical IO faults), but the recovery is wired so a partial-tags state cannot persist.
- **After step 5, mid-step 6** — commons commit is durable. Touched project files are rolled back via `git -C <project> checkout HEAD -- <path>` for each in-plan project path that has any rewrite. Per-path, not directory-level. The user is left with a commons commit and clean project trees (for promote-owned paths only — other dirty work in each project is preserved). Result: `status: failed`, `failure_stage: "rewrite_projects"`. The audit log records the commons commit hash and the partial-rewrite list for forensics. Recovery is **manual and explicit**: the user reverts the commons commit (`git -C <commons> revert <hash>`) to restore disk-wide consistency, then re-runs `--apply` with the issue (typically a project's dirty working tree) resolved. The audit log's `rollback:` block is the documented procedure. This is the only failure mode where the on-disk state is not self-consistent — keeping recovery manual avoids encoding subtle "partial promote resume" logic that would be exercised only on rare failures.
- **After step 6, mid-step 7** — extremely unlikely (writing one YAML file), but if the audit-log commit fails, the audit log path exists on disk uncommitted; everything else is fine. Result: `status: failed`, `failure_stage: "audit"`. Manual path-limited recovery: `git -C <commons> add -- .migrations/<op-id>.yaml && git -C <commons> commit -m "audit: op <op-id>" -- .migrations/<op-id>.yaml`.
...
| `test_commons_promote_discovery.py` (new) | direct `doc/papers/*.md` walk across N projects; group-by case-insensitive `bibkey_normalized`; `resolve_project_by_id` matches `.id` (not `.name`), rejects null-id entries with a clear message, rejects unregistered ids with a different message; skip files with `overlay_of:` set (already promoted); **kind-inference**: file with no `kind`/`type` is treated as paper (no silent skip); file with explicit `kind: paper` or `type: paper` is a paper; file with explicit non-paper kind (e.g. `kind: review-article`) is skipped with a warning; file with `id:` prefix ≠ `paper:` is skipped with a warning; per-file parse failure / schema-failing frontmatter produces a `FailedCandidate` and does not abort discovery; `DiscoveryResult.failed_candidates` flows into `PromotePlan.failed_candidates` |
| `test_commons_promote_plan.py` (new) | classification places only canonical fields on the canonical entity (project-only fields never appear); auto-union on disjoint canonical fields; auto-union on identical values; `FieldConflict` raised on differing values; resolver callback is honored; body-section split respects `x-canonical-body-sections`; append fields union deterministically; `failed_candidates` carried into the plan; **bibkey case**: `Huh2024` from project A and `huh2024` from project B group under one bibkey, canonical case picked deterministically from `--from` order; overlay for the case-divergent project plans a rename to canonical case |
| `test_commons_promote_apply.py` (new) | **commons preflight requires fully clean tree+index** (uncommitted OR staged change anywhere in commons before the run starts → `PromoteInputError`, run aborts); **project preflight is target-scoped** (dirty non-target files allowed; dirty target files → `PromoteInputError`); canonical paper files include `schema_profile`, `version`, `created`, `updated` set by promote; canonical paper files always carry `tags: []` (or no tags field); **canonical filename + id preserve the picked case** (e.g. `Adams2025.md` with `paper:Adams2025`, not lowercased); **case-divergent project files are renamed** to canonical case during overlay rewrite, with the rename recorded in the audit log; **date values are written as quoted ISO-8601 strings** (`created: "2026-05-15"`, not `created: 2026-05-15`), and round-trip through `yaml.safe_load` + schema validation; **tag preflight** rejects a run where any `paper/<bibkey>/<version>` tag already exists; **path-limited commit (post-preflight race)** — by injecting a staged unrelated file directly between the preflight check and step 5.2 (a controlled simulation of a TOCTOU race that preflight cannot catch), the resulting promote commit contains ONLY the canonical paper files; **mid-step-5 simulated tag failure** is fully reverted using `reset --soft` + per-path `checkout HEAD --` (or unlink for first-promote files); a file injected as staged AFTER preflight but BEFORE the tag failure is still staged afterward — would have been clobbered by `reset --hard`; **path-limited audit commit (post-preflight race)** — same TOCTOU-race regression test for `.migrations/` commit; single commit + N tags pointing at it; project file rewrites with pin to tagged version; rewritten overlays validate against `overlay-1.1`; rewritten overlays preserve the project's original `created` / `updated`; audit log shape (success path: committed; failure path: best-effort uncommitted); **failure-mid-rewrite rolls back project files but leaves commons commit**; **project rollback is path-limited** — dirty non-target files in the project are NOT affected by the mid-step-6 rollback; **failure-before-commit leaves nothing on disk** (path-limited restore for re-promote files, unlink for first-promote files; no directory-level `checkout`); idempotent re-apply (an already-overlayed file is skipped, no-op); `failed_candidates` surfaces in the audit log |
