...
**Goal:** Reconcile the `science curate inventory` contract in `commands/curate.md` (and its generated Codex mirror) to what the helper actually returns, implementing the fb-2026-07-10-017 correction pending merge, and correct the three historical docs that mis-state that id.
...
- [ ] **Step 4: Transition the governing design from Decision-ready to implemented and correct its title**

Everything else now declares fb-2026-07-10-017 implemented pending merge; the governing design must not stay `Decision-ready`. Use SHIPPED/CLOSED only post-merge (attention-recency precedent) — on this branch it is *implemented, pending merge*. State the consequence of merging in the future tense.

In `docs/plans/2026-07-16-curate-inventory-contract-reconciliation-design.md`,
replace the complete opening title-and-status block with this Edit. The old
snippet's terminal wording is the defect being removed. `old_string`:
