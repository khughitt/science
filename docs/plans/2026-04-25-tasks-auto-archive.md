...
**Goal:** Add `science tasks archive` so done/retired entries in `tasks/active.md` are moved to `tasks/done/YYYY-MM.md` deterministically, and surface archive lag in `science health`, eliminating the operational drift seen in 3 of 4 audited downstream projects.
...
**Architecture:** Reuse the existing `science_tool.tasks` parser/renderer. Add a focused `science_tool.tasks_archive` module that owns scan, classify, route-by-completed-date, and idempotent file writes. Expose it via the existing `tasks` Click group as `science tasks archive`. Surface drift counts under a new `tasks.archive_lag` block in the health report so `/science:status` and `/science:next-steps` can prompt the user to run the archiver.
...
- **Health-only mode.** `--check` emits archivable counts as JSON and exits **non-zero (exit 1)** when any of `done_in_active` / `retired_in_active` / `missing_completed` is non-zero; exits 0 when clean. Used by both `science health` (which interprets the exit code) and CI gates that want to fail on lag. The health-report consumer reads counts from JSON output regardless of exit code.
...
- `science/src/science_tool/tasks_archive.py` — archiver module: `ArchivePlan`, `ArchiveEntry`, `plan_archive`, `apply_archive`, `count_archivable`. (Module name parallels `tasks_display.py` / `datasets_register.py`; the package style for tasks is flat-module, not subpackage. Do not create a `tasks/` subpackage — that would shadow `tasks.py`.)
- `science/tests/test_tasks_archive.py` — unit + CLI tests covering dry-run, apply, idempotency, completed-date routing, missing-completed fallback, header preservation, mixed status values, malformed entries, JSON output.

Modify:

- `science/src/science_tool/cli.py` — add `tasks archive` subcommand under the existing `@main.group() tasks` group (line 1928). Reuses `OUTPUT_FORMATS` and the `emit_query_rows` helper used by other commands.
- `science/src/science_tool/graph/health.py` — add `TaskArchiveLag` TypedDict, add `archive_lag` field to `HealthReport`, populate it in `build_health_report` from the new archiver.
- `science/tests/test_health.py` — health-report integration test that creates an `active.md` with one done + one retired + one proposed entry and asserts the lag counts.
- `commands/status.md` — add an "archive lag" bullet under §6 "Staleness Warnings" so `/science:status` calls out non-zero `tasks.archive_lag` from `science health`.
- `commands/next-steps.md` — under "Task Tracking Gaps", note that any non-zero `tasks.archive_lag` should produce a Recommended Next Action: run `science tasks archive --apply`.
...
**Files:** Create `science/src/science_tool/tasks_archive.py`; create `science/tests/test_tasks_archive.py`.
...
- [x] **Step 3: Run** `cd science && uv run --frozen pytest tests/test_tasks_archive.py -q -k planner` — expect pass.
...
**Files:** Modify `science/src/science_tool/tasks_archive.py` and `science/tests/test_tasks_archive.py`.
...
- [x] **Step 4: Run** `cd science && uv run --frozen pytest tests/test_tasks_archive.py -q` — expect pass.
...
**Files:** Modify `science/src/science_tool/cli.py` and `science/tests/test_tasks_archive.py`.
...
@click.option("--check", is_flag=True, help="Print archivable counts and exit (used by science health).")
...
- [x] **Step 3: Run** `cd science && uv run --frozen pytest tests/test_tasks_archive.py -q` — expect pass.
...
**Files:** Modify `science/src/science_tool/graph/health.py`, `cli.py`, `tests/test_health.py`.
...
- [x] **Step 3: Render in `health_command`** — before the unresolved-refs table, render a "Tasks Archive Lag" `Table` if `any(archive_lag.values())`, append a "Next: run `science tasks archive`" hint, and include archive lag in `total_issues` when non-zero.

- [x] **Step 4: Run** `cd science && uv run --frozen pytest tests/test_health.py tests/test_tasks_archive.py -q` — expect pass.
...
- [x] **Step 1:** In `commands/status.md` §6 "Staleness Warnings", append a bullet recommending `science tasks archive --apply` when `tasks.archive_lag.{done_in_active, retired_in_active}` is non-zero.

- [x] **Step 2:** In `commands/next-steps.md` under "Task Tracking Gaps (if any)", append guidance: when `science health --format json` shows non-zero `archive_lag.done_in_active` or `archive_lag.retired_in_active`, add a Recommended Next Action — preview with `science tasks archive`, then `--apply`. Call out `missing_completed` entries so the user backfills `completed:` first.
...
- [x] **Step 1: Focused tests** — `cd science && uv run --frozen pytest tests/test_tasks_archive.py tests/test_health.py tests/test_tasks_cli.py tests/test_tasks.py -q` — expect pass.

- [x] **Step 2: Format and lint** — `uv run --frozen ruff format` + `ruff check` over `science/src/science_tool` — expect no errors.
...
- Health closure: `archive_lag` block surfaces in `science health`, `/science:status`, and `/science:next-steps` so the convention has both an enforcement mechanism and a reminder loop.
