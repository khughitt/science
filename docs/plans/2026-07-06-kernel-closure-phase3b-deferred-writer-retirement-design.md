...
**Convert the template to Renderer format (required by `template_ready`).** `template_ready=True` routes `entity create` through the migrated Renderer, which **hard-fails any template lacking a `_template` metadata block** (`templates.py:181`). The current `templates/story.md` is plain frontmatter with no `_template` block, so it must be **converted** (not merely edited) to the Renderer format — a `_template.frontmatter` map + `_template.sections` list, modeled on `templates/evidence-line.md` — in both the packaged copy (`science_model/templates/story.md`) and the repo-root mirror (`templates/story.md`). While converting, **remove the misleading `about:`/`interpretations:` frontmatter keys** (they produce no edges under generic materialization) so the scaffold doesn't imply an edge path that doesn't exist. Instead, the template body and the retirement message must point authors at a concrete `relations.yaml` snippet, e.g.:
...
**Typed-model registration (must be complete or the materializer branch never fires).** The graph build instantiates typed entities through the **`CORE_KIND_MODELS` registry**, not `parse_entity_file`: `load_project_sources` calls `registry.resolve(kind)` then `schema.model_validate(raw)` (`sources.py:358,375`), and `EntityRegistry.with_core_types()` binds each kind to `CORE_KIND_MODELS.get(name, ProjectEntity)` (`entity_registry.py:55,~90`). A kind absent from `CORE_KIND_MODELS` loads as a bare `ProjectEntity`, so the `isinstance(entity, FalsificationEntity)` branch would never fire. Registration therefore touches every one of:
- `entities.py` — `class FalsificationEntity(ProjectEntity)` with fields `falsifies`, `predicted`, `observed`, `decision`, `source_of_prediction`, `supersedes_claim` (mirror `EvidenceLineEntity`, `entities.py:950`). Fields must be named to match frontmatter keys so `model_validate(raw)` populates them (the build path uses `model_validate`, not explicit kwargs).
- `entities.py` — `EntityType.FALSIFICATION = "falsification"` enum member.
- **`graph/entity_registry.py` — add `"falsification": FalsificationEntity` to `CORE_KIND_MODELS`** (+ import). *This is the load-bearing registration for the build path.*
- `frontmatter.py` — add the inline `if kind == EntityType.FALSIFICATION.value: return FalsificationEntity(...)` branch in `parse_entity_file` (`:334`, alongside the `EVIDENCE_LINE` branch at `:424`). This is the **secondary** path (consumed by `plan_gate.py`), not the graph build — register it too for consistency, but it is not what makes materialization work.
- `science_model/__init__.py` — export `FalsificationEntity`; `materialize.py` — import it for the `isinstance` dispatch.
- Tests: a **load-source integration test** (author `entities/falsifications/x.md`, run `load_project_sources`, assert the loaded entity `isinstance` `FalsificationEntity` — proves the registry binding, the real failure mode), plus a `parse_entity_file` round-trip test.
...
**Template** `science_model/templates/falsification.md` (+ repo-root mirror), authored in **Renderer format** (a `_template.frontmatter` map + `_template.sections` list, modeled on `templates/evidence-line.md` — `template_ready` requires the `_template` block, `templates.py:181`), with a `falsifies:` frontmatter field and sections cueing predicted/observed/decision.
...
- **Renderer/create coverage** — `science entity create story`, `science entity create falsification`, and `science entity sections story` / `… falsification` must succeed (they exercise the `_template` block); add these alongside 3a's `entity create` template tests. The `MIGRATED_KINDS`/reconciliation gates already assert both kinds render.
...
2. **Build forward paths** — `FalsificationEntity` schema + **full typed-model registration** (EntityType member, **`CORE_KIND_MODELS` binding — the build-path registration**, `parse_entity_file` branch, `__init__` export, `materialize` import — §3.4) + descriptor + **Renderer-format** `falsification.md` template + **`knowledge`-routed** compiler emitters (with the named-graph parity test + the load-source `isinstance` test); wire the `story` kind (home/template_ready/strategy + **`draft/developing/mature` statuses** + **convert `story.md` to Renderer `_template` format** with the corrected keys, pointing at `relations.yaml`); update the reconciliation-gate literals.
