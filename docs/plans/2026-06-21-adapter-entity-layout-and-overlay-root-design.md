...
- `docs/user-guide/entities.md#dataset-lifecycle` — records the dataset lifecycle contract that earlier deferred "should dataset/workflow/workflow-run become first-class `entities/` kinds" to a dedicated follow-up. **This is that follow-up** (the layout half only).
