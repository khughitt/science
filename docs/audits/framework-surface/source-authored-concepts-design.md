...
| A `terms.yaml` (or aggregate) row and a new markdown owner both mint the same `concept:*` id. | The destination check (`entities.py`) only tests file existence, so it cannot see a same-id row in another source. Confirm the existing identity-collision path covers the two-tier case: `MarkdownAdapter` precedes `AggregateAdapter` (which loads `terms.yaml`) in the source-loader adapter list, so the markdown owner claims the id first and the `terms.yaml` row is the duplicate. A competing row raises `EntityIdentityCollisionError` under `strict_identity`; otherwise the markdown owner wins and the aggregate row is a deprecated-owner collision — a non-blocking `warn` in `science validate` / the `entity_identity` health check, not a build failure. Add a test that asserts this rather than assuming it. |
