...
2. Add a test or lint for new commands that expose `--json` without `--format`,
   ...
   **Status:** Added in `science/tests/test_cli_surface_contract.py`.
   `project verify`, `data audit`, `qa-audit`, `book-split`, `bib add`, and
   legacy Commons JSON commands now accept `--format json` while preserving
   `--json`; the JSON-only allowlist is empty.
