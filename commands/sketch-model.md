...
to satisfy a sketch. If no supported durable source kind exists yet, keep the
term described in the inquiry patch prose and either connect it to an existing
source ref or defer the boundary/edge until a source owner is available.

Do not apply the broad rule "For inquiry-internal variables, declare the refs in
`entities/patches/<slug>.md`" to every inquiry field. `boundary_roles`,
`flow_edges`, and `unknowns` should reference existing source refs. The inquiry
compiler mints inquiry-local assumption and transformation nodes from the patch
source; it does not make fresh boundary or flow endpoint refs durable. Do not
use `science graph add concept` as the durable authoring path. Direct graph
mutation writes to `knowledge/graph.trig`, which is regenerated by `science
graph build` from source files.
