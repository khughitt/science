...
_BATTERY: dict[str, list[Any]] = {
    ...
    # NO search record authors `profile`; the loader injects it (graph/sources.py:1041).
    # `core` is ProjectEntity's DEFAULT, and here -- unlike concept -- it is also what the
    # graph shows on real search records, so it is a live value rather than a model default.
