...
def load_trig_dataset_preserving_literals(graph_path: Path) -> Dataset:
    ...
    conjunctive_graph = ConjunctiveGraph(store=default_graph.store, identifier=default_graph.identifier)
    conjunctive_graph.default_context = default_graph
    conjunctive_graph.namespace_manager = default_graph.namespace_manager
    ...
    try:
        ...
        sink = _LiteralPreservingRDFSink(conjunctive_graph)
        base_uri = conjunctive_graph.absolutize(source.getPublicId() or source.getSystemId() or "")
        ...
        for prefix, namespace in parser._bindings.items():
            conjunctive_graph.bind(prefix, namespace)
    finally:
