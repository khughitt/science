...
ROLE_DIRECT_TEST = EvidenceRole.DIRECT_TEST.value
ROLE_PROXY_SUPPORT = EvidenceRole.PROXY_SUPPORT.value
ROLE_BACKGROUND = EvidenceRole.BACKGROUND_CONSTRAINT.value
ROLE_NEGATIVE_CONTROL = EvidenceRole.NEGATIVE_CONTROL.value
ROLE_MODEL_CRITICISM = EvidenceRole.MODEL_CRITICISM.value
...
# Keyed on canonical string tokens from the model StrEnums.
EVIDENCE_TYPE_RANK = {
    EvidenceType.EMPIRICAL_DATA.value: 4,
    EvidenceType.BENCHMARK.value: 3,
    EvidenceType.SIMULATION.value: 3,
    EvidenceType.LITERATURE.value: 2,
    EvidenceType.EXPERT_JUDGMENT.value: 1,
}
# Valid-but-unranked-by-design types (rank 0), parallel to diagnostic roles.
UNRANKED_EVIDENCE_TYPES = frozenset({EvidenceType.NEGATIVE_RESULT.value})
EVIDENCE_ROLE_RANK = {
    EvidenceRole.DIRECT_TEST.value: 3,
    EvidenceRole.PROXY_SUPPORT.value: 2,
    EvidenceRole.BACKGROUND_CONSTRAINT.value: 1,
}
STRENGTH_RANK = {
...
def _reconcile_evidence_vocab() -> None:
    ...
    if set(EVIDENCE_TYPE_RANK) | UNRANKED_EVIDENCE_TYPES != {item.value for item in EvidenceType}:
        raise ValueError(
    ...
    if set(EVIDENCE_ROLE_RANK) != {item.value for item in EvidenceRole} - DIAGNOSTIC_ROLES:
        raise ValueError("EVIDENCE_ROLE_RANK must rank exactly the non-diagnostic EvidenceRoles")
    if set(STRENGTH_RANK) != {item.value for item in EvidenceStrength}:
