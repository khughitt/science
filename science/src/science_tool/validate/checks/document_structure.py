...
def check_document_structure(ctx: ValidateContext) -> Iterator[Result]:
    yield from _check_documents(ctx, ctx.doc_dir / "background" / "topics", _TOPIC_SECTIONS)
    yield from _check_documents(ctx, ctx.doc_dir / "background" / "papers", _PAPER_SECTIONS)
