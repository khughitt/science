...
def check_prereg_vehicles(ctx: ValidateContext) -> Iterator[Result]:
    ...
    for path in sorted(entities_root.glob("*.md")):
        ...
        if not entries:
            frozen = str(frontmatter.get("status", "")) in _FROZEN_STATUSES
            if frozen and _DATA_GATED_MARKER not in ctx.body(path):
                yield _result(
                    Severity.WARN,
                    relative,
                    f"{relative} is {frontmatter.get('status')} but declares no 'vehicles:'. A "
