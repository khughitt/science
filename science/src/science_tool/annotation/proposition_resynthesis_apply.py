...
def _resume_validation_report(
    ...
    *,
    ...
    if draft.original_proposition in replacements:
        raise ResynthesisApplyError("original proposition cannot be a replacement")
    ...
    already_promoted_missing = sorted(
        annotation_ref
        for annotation_ref, (_sidecar_path, _sidecar, promoted_to) in live_index.items()
        if promoted_to in replacements
        and (annotation_ref not in expected_action_annotations or annotation_ref not in seen_annotations)
    )
    ...
    if set(replacements) != assigned_replacement_ids:
        raise ResynthesisApplyError("replacement propositions must match assigned annotation targets")
