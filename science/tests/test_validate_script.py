...
def _validate_script_path() -> Path:
    # Target the canonical validate.sh shipped with science. The
    # repo-root scripts/validate.sh is a thin shim that delegates to
    # `science project artifacts exec validate.sh`; these tests
...
def test_validate_reports_missing_science_tool_once_when_graph_exists(tmp_path: Path) -> None:
    ...
    message = "science is required for task management, feedback, and graph workflows"
    assert result.returncode != 0
