...
def test_generated_project_setup_skills_declare_boundary_not_data_negations(
...
def test_generated_project_setup_skills_preserve_boundary_workflow_order(
    generated: GenerationResult,
) -> None:
    create_text = generated.skill_paths["science-create-project"].read_text(
        encoding="utf-8",
    )
    ...
    assert (
        "only when `science.yaml` declares one or more `boundary.roots` entries"
        in _norm(initialize_git)
    )
    assert (
    ...
    import_text = generated.skill_paths["science-import-project"].read_text(
        encoding="utf-8",
    )
...
def test_next_steps_skill_queries_done_ledgers_for_each_month_in_recent_window(
    skills_root: Path,
) -> None:
    text = _norm(_read_skill(skills_root, "science-next-steps"))
