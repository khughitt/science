...
""",
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["peers", "migrate", "--project-root", str(child)])

    assert result.exit_code == 0, result.output
    raw = yaml.safe_load((child / "science.yaml").read_text(encoding="utf-8"))
    assert "parent" not in raw
    assert raw["peers"] == [{"id": "meta-project", "path": "../meta"}]
    assert f"migrated: {child}" in result.output
...
""",
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["peers", "migrate", "--project-root", str(child), "--dry-run"])

    assert result.exit_code == 0, result.output
    assert (child / "science.yaml").read_text(encoding="utf-8") == original_text
    assert f"would migrate: {child}" in result.output
...
""",
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["peers", "migrate", "--project-root", str(meta), "--all"])

    assert result.exit_code == 0, result.output
    host_raw = yaml.safe_load((meta / "science.yaml").read_text(encoding="utf-8"))
    assert "children" not in host_raw
    assert host_raw["peers"] == [{"id": "child-project", "path": "../child"}]
    child_raw = yaml.safe_load((child / "science.yaml").read_text(encoding="utf-8"))
    assert "parent" not in child_raw
    assert child_raw["peers"] == [{"id": "meta-project", "path": "../meta"}]


def test_peers_migrate_all_missing_child_path_fails_without_writing(tmp_path: Path) -> None:
    meta = tmp_path / "meta"
    meta.mkdir()
    _write_yaml(
        meta,
...
""",
    ...
    original_text = (meta / "science.yaml").read_text(encoding="utf-8")
    ...
    assert (meta / "science.yaml").read_text(encoding="utf-8") == original_text


def test_peers_migrate_all_child_failure_preflight_leaves_host_unchanged(tmp_path: Path) -> None:
    meta = tmp_path / "meta"
    child = tmp_path / "child"
    meta.mkdir()
    child.mkdir()
...
""",
    ...
    original_host_text = (meta / "science.yaml").read_text(encoding="utf-8")
    original_child_text = (child / "science.yaml").read_text(encoding="utf-8")

    runner = CliRunner()
    result = runner.invoke(main, ["peers", "migrate", "--project-root", str(meta), "--all"])

    assert result.exit_code != 0
    assert "children must be a list" in result.output
    assert (meta / "science.yaml").read_text(encoding="utf-8") == original_host_text
    assert (child / "science.yaml").read_text(encoding="utf-8") == original_child_text


def test_peers_migrate_single_error_is_wrapped_as_cli_error(tmp_path: Path) -> None:
    child = tmp_path / "child"
    child.mkdir()
    _write_yaml(
        child,
...
""",
    ...
    assert result.exit_code != 0
    assert "error:" in result.output.lower()
    assert "cannot migrate parent" in result.output
