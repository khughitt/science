...
def test_load_dag_paths_software_profile_without_block_uses_defaults(tmp_path: Path) -> None:
    """Profiles without an explicit dag: block fall back to defaults; zero DAGs is a valid empty state."""
    (tmp_path / "science.yaml").write_text("profile: software\n")
    paths = load_dag_paths(tmp_path)
    assert paths.dag_dir == tmp_path / "doc/figures/dags"
    assert paths.tasks_dir == tmp_path / "tasks"
    assert paths.dags is None  # auto-discover; finds zero when nothing is there
