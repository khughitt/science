...
def test_cli_dag_render_single_slug(cli_project: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "render", "--dag", "h1-progression",
...
def test_cli_dag_staleness_json_schema(cli_project: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "staleness", "--json",
                                   "--project", str(cli_project)])
    ...
    assert {"today", "recent_days", "drifted_edges", "under_reviewed_edges",
            "unresolved_refs", "unpropagated_tasks"} <= set(data.keys())
...
def test_cli_dag_staleness_exit_code_on_clean_project(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "staleness",
                                   "--project", str(project)])
    assert result.exit_code == 0
...
def test_cli_dag_audit_fix_mutates(cli_project: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "audit", "--fix",
                                   "--project", str(cli_project)])
...
def test_cli_dag_init_scaffolds_new_dag(cli_project: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "init", "h3-new-hypothesis",
                                   "--label", "H3 New",
                                   "--project", str(cli_project)])
    assert result.exit_code == 0, result.output
...
def test_cli_dag_init_refuses_to_overwrite_existing(cli_project: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["dag", "init", "h1-prognosis",  # already exists
                                   "--project", str(cli_project)])
    assert result.exit_code != 0
