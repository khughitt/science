...
`boundary_roles`, `flow_edges`, and `unknowns` should reference existing source
refs. Use the patch source for inquiry-local assumptions and transformations;
the inquiry compiler mints those local nodes from the authored patch. Do not use
`science graph add concept` as the durable authoring path. Direct graph mutation
writes to `knowledge/graph.trig`, which is regenerated by `science graph build`
from source files.
