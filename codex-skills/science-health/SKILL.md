...
Before proposing new entities or ref rewrites, read `docs/process/entity-creation-cookbook.md`.
...
- **looks_like=semantic-triage**: refs that need semantic classification before any rewrite
- **looks_like=topic**: legacy topic-like refs that should usually be reclassified, retired, or kept as prose-only notes
- **looks_like=unknown**: anything else

For the semantic-triage / topic clusters, sub-cluster by likely disposition:
- domain or catalog entity
- method
- project-local `concept:*`
- `mechanism:*`
- prose-only fallback
- operational metadata that should be dropped or rewritten

Use the cookbook to decide whether a missing ref actually deserves entity status. Triage against:
- semantic kind
- external-id requirement
- prose-only fallback

For legacy shorthand values, sub-cluster by user judgment hint:
...
Unresolved References (N total):
  ...
  - 12 need semantic triage (typed entity vs prose-only)
  - 8 look like operational markers (consider meta: prefix or drop)
...
> "12 refs look like real semantics but not stable `topic` entities: topic:genomics, topic:protein, topic:embeddings, ... Reclassify these using the cookbook into typed entities, local concepts, mechanisms, or prose-only notes?"
...
- Creating typed entities: only after cookbook triage confirms a stable semantic kind and any required external ids
- Migrating tags: `science-tool graph migrate-tags --apply` (default meta:)
- Migrating tags as topics: `science-tool graph migrate-tags --apply --as-topic` only for explicit legacy cleanup, not as a default recommendation
...
- Do not recommend generic entity stubs. If a ref does not pass semantic triage, keep it prose-only or remove it.
