...
This tool probes a fixed tiered list of agent-friendly sources (Crossref → Unpaywall → arXiv → bioRxiv/medRxiv → Europe PMC → direct OA PDF) with per-host rate limiting shared across all concurrent subagents. **Branch strictly on the `status` field it returns — do not run an open-ended web search as a default fallback.** A DOI→PMCID→Europe PMC full-text probe runs *before* `paper-fetch` decides `paywalled`, so PMC-hosted NIH author manuscripts that Unpaywall mis-flags as closed are recovered automatically — never conclude "paywalled" from your own WebFetch of a publisher landing page (e.g. ScienceDirect returning metadata only); that verdict is `paper-fetch`'s to make, and it has already consulted PMC.
...
| `paywalled` | Unpaywall reports no OA copy **and** the DOI→PMCID→Europe PMC full-text tier already came up empty — so PMC has been checked; do not re-WebFetch the publisher page to "confirm". Default: stop and report back. **If you have independent reason to think an OA copy exists** (an NIH-funded paper, or the user references a PMC URL / PMCID / `nihms-*` file), re-run `paper-fetch` with `--url <pmc-url>` or `--pmcid <PMCID>` — it may recover full text Unpaywall missed. **Well-known-classic exception** — if the paper is a well-known classic (year ≤ current_year − 3, >500 citations, conceptual task, comprehensive LLM coverage), you may proceed with `Source: LLM knowledge` and generous `[UNVERIFIED]` markers; never invent quantitative claims. For paywalled review papers, triangulate via 2-3 citing primary papers (Europe PMC citations endpoint) instead of relying on the abstract alone. |
