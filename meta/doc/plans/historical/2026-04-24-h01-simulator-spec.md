...
Design spec for the simulator used to test `hypothesis:0001-stochastic-revisiting`.
...
**Bias model.** To distinguish independent analyst noise from shared-pipeline error (the H01 disputing-evidence route; see `question:0001-bioinformatics-generalizability`), the signal model supports three modes:
...
- **P2 (realistic regime)** — tested by whether the advantage appears within the defensible subset of the noise grid; requires cross-referencing `question:0001-bioinformatics-generalizability` for plausibility of noise settings.
