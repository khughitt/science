...
- If most real-world scientific error is shared-pipeline bias (the `question:0001-bioinformatics-generalizability` angle), the stochastic-revisiting mechanism may help little in any regime. A hypothesis about *detecting* pipeline-level correlated error across claims might be a higher-value first test.
