...
def test_hard_gate_excludes_below_threshold():
    policy = hard_gate_policy(
        PolicyConfig(kind="hard_gate", warmup_actions=0, gate_threshold=0.5)
    )
    m = _fresh_model(n=3)
    for _ in range(10):
...
def test_hard_gate_falls_back_when_all_gated():
    policy = hard_gate_policy(
        PolicyConfig(kind="hard_gate", warmup_actions=0, gate_threshold=0.99)
    )
    m = _fresh_model(n=3)
    rng = np.random.default_rng(0)
...
```python
from h01_simulator.policies import thompson_policy
...
def test_thompson_output_is_a_valid_index():
    policy = thompson_policy(PolicyConfig(kind="thompson", warmup_actions=0))
    m = _fresh_model(n=5)
    rng = np.random.default_rng(0)
    for _ in range(100):
