...
4. **Should a payload list per-extension `reason_codes` separately, or one merged list?** Spec'd as per-extension above (each extension contributes; effective codes = union). Alternative: only `core.reason_codes` is authored, extensions only contribute static "always-on" codes. Decision: authored codes are core; extension-static codes added via the registry. Confirm with `[t030]`.
...
- **Validate authoring cost.** Hand `[t030]` a sample of 10–20 existing paper summaries; have a human and an LLM each draft payloads under this schema; record per-field success/ambiguity rate. Surfaces field overload (fields nobody can author) and field drift (fields people invent because the schema is missing them).
...
Field names and enum values remain candidates pending `[t030]` empirical signal. The structural decisions (core/extension split, multi-extension dispatch, reason-code inheritance) are the load-bearing claims of this draft.
