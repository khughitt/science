...
5. **Latent-construct modeling now or later** — bias *correction* (latent proxy model) in scope, or only bias *flagging*? ✅ **IN SCOPE, prototyped (t066, 2026-06-01):** a PMI / two-way-independence measurement model *subtracts* the publication-attention axis (not merely flags it) on the real q14 slice — `meta/src/h00_patch_l1/latent.py`, `interpretation:h00-t066-latent-correction-2026-06-01`. The per-gene attention vector `α_g` doubles as the §2 GLUE latent axis (PPMI factorization → t067). Scale validation + variance guard = t070.
