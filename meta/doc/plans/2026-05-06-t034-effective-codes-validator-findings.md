...
**Known divergence.** Slice-3 prototype's test 14 (`test_strengthen_mr_two_stage_iar_finding`) was authored under v1.3 to assert that iar misfires on the two-stage MR pattern (no retirement rule in v1.3). Under v1.4 P1.4-a, that misfire is fixed — the production module retires iar correctly, as confirmed by the smoke test. The prototype itself remains in v1.3 form; aligning it to v1.4 (mirror of the slice-2 alignment landed earlier) is a small follow-up patch that flips test 14's expectation.
