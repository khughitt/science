...
1. ~~**Land the slice-2 prototype patch.**~~ **Done 2026-05-09.** Slice-2 prototype now drops `mr-6` and replaces it with `v1.3-auto-inject` as a co-validator; effective-codes computation adds the auto-injection step; fixtures `_base_payload`, tc08, tc11, tc13, tc14, tc15, tc17, tc20, tc24 strip hand-written `instrument-assumption-risk`; tc21 repurposed to verify hand-writing iar fires `v1.3-auto-inject`. 25/25 tests still pass. The "prototype lags contract" gap from v1.3 (commit 85b984c) is now closed.
...
4. ~~**Fold the prototypes into `meta/validate.sh`.**~~ **Done 2026-05-09.** Production module landed at `meta/src/t034_validator/` (single package, ~470 lines + ~50-line YAML loader + ~35-line CLI). The three slice rule sets are bundled with v1.4 in baked: P1.4-a iar retirement is implemented in `_retired_by` (the first retirement rule reading upstream state via `mr_graph_payload_ref`), and v1.3 P1.3-c hand-written-auto-injected codes hard-error per P1.4-b. `validate.local.sh` registers an `extra_checks` hook that runs `python -m t034_validator evidence/` and fails validate.sh on any error. End-to-end smoke tests confirm: T34-6 stage (b) two-stage MR pattern now validates at strengthen-belief; the unhandled-pleiotropy negative case correctly fires `cee-strengthen-b`; empty payload directory passes silently. `evidence/` is the payload-directory convention; one payload per `.yaml` / `.yml` file, keyed by `core.payload_id`. PyYAML 6.0+ added as a direct dependency. Three prototype files remain in `doc/plans/` as test fixtures and design records.
...
**Slice-3 prototype now in v1.4 sync (2026-05-09).** The prototype's `_retired_by` was extended to read upstream `mr_graph_payload_ref.instrument_validity_assumptions` for the P1.4-a iar retirement; test 14 was renamed `test_strengthen_mr_two_stage_iar_retired` and its fixture populated with the upstream relevance assumption + `mr_graph_payload_ref`, with the assertion flipped from "expects cee-strengthen-b" to "expects no errors." 14/14 still pass. All three slice prototypes now match the v1.4 contract that the production validator at `meta/src/t034_validator/` enforces.
