...
Six question tracks define the representation prerequisite: the evidence payload must capture typed reason codes before any attention policy can consume them. Those tracks are `question:01-evidence-payload-schema`, `question:03-source-and-pipeline-provenance`, `question:10-causal-graph-construction-pipeline`, `question:11-graph-valued-synthesis-artifacts`, `question:12-agent-tool-kg-operations`, and `question:13-robustness-reproducibility-evaluation`.
...
**Live questions.** All six linked questions remain open at the representation level. `question:01-evidence-payload-schema` is the bottleneck: reason codes cannot be assigned or consumed until the payload schema is designed. `question:03-source-and-pipeline-provenance`, `question:10-causal-graph-construction-pipeline`, `question:11-graph-valued-synthesis-artifacts`, `question:12-agent-tool-kg-operations`, and `question:13-robustness-reproducibility-evaluation` each define a reason-code family that feeds the H03 policy.
