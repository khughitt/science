...
Implement the engine per `doc/plans/historical/2026-04-24-h01-simulator-spec.md`: Beta-Bernoulli signal model with configurable prior and three bias modes (none / independent / shared); three policies (hard-gate, constant-revisit, Thompson); recall / Brier / regret metrics; grid sweep producing a list-column parquet (allocations + final α, β arrays); Click CLI with a benchmark gate that validates runtime against the single-digit-minute budget. Quality gates: ruff check + format-check + pyright. Plan: `doc/plans/historical/2026-04-24-h01-simulator.md`. Running the full sweep, populating notebook figures, and writing the interpretation are deliverables of [t002].
...
- parent: task:t001
- aspects: [software-development, hypothesis-testing]
- related: [hypothesis:0001-stochastic-revisiting, task:t001]
- created: 2026-04-24

Resolve the three engine follow-up issues tracked in `meta/doc/plans/historical/2026-04-24-h01-engine-followups.md`: (1) parameterise the default grid in dimensionless budget-multiples with degenerate-cell filtering; (2) rename `regret` to `signal_count_regret` and document its decorrelation from recall under shared bias at high noise (no companion metric added; a budget-aware recall oracle is deferred until interpretation needs it); (3) parallelise `run_sweep` via `ProcessPoolExecutor` with validated `workers >= 1`, sample `benchmark_runtime` calibration stratified by `(n_propositions, budget)`, and re-anchor `RUNTIME_BUDGET_SECONDS` to honest serial CPU. Unblocks [t002].
...
Surfaced by: code-review pass on the 2026-04-25 tasks auto-archive rollout, now implemented in `science/src/science_tool/tasks_archive.py` (master rollout follow-on action #3).
...
Implement Phase 1 of `docs/plans/historical/2026-05-03-epistemic-dependency-graph-design.md`: explicit epistemic/operational/reference entity taxonomy on `EntityKind`, the `bears_on` relation kind with auto-derivation rules, and per-entity `EpistemicFreshness` state with a graph-build propagation step.
...
Phase 2 of `docs/plans/historical/2026-05-03-epistemic-dependency-graph-design.md`: replace deterministic top-N selection in `science:next-steps`, `science:curate`, `science:big-picture`, and task-prioritization sweeps with **weighted random sampling** over candidate epistemic entities. Weight function uses observable graph properties (incoming `bears_on` count, days-since-review, freshness state, evidence-imbalance skew) plus an `ε` floor so nothing collapses to zero — operationalizing D-003 at the attention layer.
...
- related: [hypothesis:0001-stochastic-revisiting, question:0002-evidence-payload-schema, question:0003-causal-synthesis-guardrails]
...
- parent: task:t021
- aspects: [software-development, framework-design, hypothesis-testing]
- related: [task:t021, question:0002-evidence-payload-schema, hypothesis:0001-stochastic-revisiting, topic:bayesian-methods-continuous-belief]
- group: evidence-payload-schema
- created: 2026-05-05
...
- related: [task:t022, question:0005-authoring-cost-audit, hypothesis:0002-rich-evidence-payloads-improve-graph-calibration]
- group: evidence-payload-schema
- created: 2026-05-05
- completed: 2026-05-06
...
- parent: task:t021
- aspects: [software-development, framework-design, hypothesis-testing]
- related: [task:t021, task:t024, task:t025, task:t026, task:t031, task:t034, task:t035, task:t037, task:t038, task:t040, task:t042, question:0002-evidence-payload-schema, topic:bayesian-methods-continuous-belief]
- group: evidence-payload-schema
- created: 2026-05-05
...
Design draft: `doc/plans/historical/2026-05-07-t023-typed-synthesis-nodes-design.md`.

v1 design recorded in doc/plans/historical/2026-05-07-t023-typed-synthesis-nodes-design.md; review fixes clarified synthesis nodes as t022 payloads, split default/max permissions, added routing/arbitration rules, scoped effective reason codes, and tracked lifecycle follow-up as t042.
...
Created question:0014-adaptive-project-topology and hypothesis:0006-adaptive-project-topology-improves-research-fit.
...
Implemented overlay/skip-already-present for commons promote (paper kind) + fb-002 auto-reindex + fb-003 failure-audit commit. Merged to main at 49d38cfa (10 commits). Full suite green; unblocks MM bundled promote.
...
Foundational fork resolution for h00 working model (RFC: doc/plans/historical/2026-05-31-epistemic-causal-probabilistic-graph-model-design.md §12). Decide: (1) reuse t034 causal-graph substrate verbatim (graph_object_type/epistemic_role/identification pipeline) — recommended; (2) storage substrate for patch + world<->claim reification + multi-edges: RDF-star/named-graphs/PROV-O vs labeled-property-graph vs hybrid. Everything downstream hangs off these.

Resolved via D-005 (reuse t034 verbatim; h00 net-new rides t022 extension contract) + D-006 (W3C-native: patch=named graph, world<->claim reification = existing edge-as-node pattern, not RDF-star; LPG rejected). RFC §12.1/§12.2 marked DECIDED. Substrate decisions grounded in the live graph.trig (already RDF/TriG named graphs + PROV-O + reified edge nodes), so both were confirmations of the running substrate, not greenfield choices. Patch-graph nesting deferred to t067.
