...
## [t063] commons promote: overlay/skip papers already in the commons (cross-project dedup against committed content)
- priority: P2
- status: proposed
- created: 2026-05-22

commons promote --apply hard-codes canonical_version=1.0.0 and dedups slugs only within a single multi-from invocation, so it aborts on any paper/<slug>/1.0.0 tag that already exists in the commons. Once one project in a family is promoted, a sibling that shares any paper (a genuinely shared paper OR a true bibkey collision) cannot be promoted independently. Add an overlay/skip-already-present path: when a canonical entity for a slug already exists, rewrite the source summary as an overlay pinned to the existing version (do not re-mint or abort). This unblocks promoting multiple-myeloma (shares ~5 papers with already-promoted evolution and pan-disease) and any future project-by-project promotion. Filed as science feedback fb-2026-05-22-001 (target command:commons-promote). Companion operational gaps filed as fb-2026-05-22-002 (no registry.sqlite reindex after --apply), -003 (orphan .migrations audit log blocks retries), -004 (ensure_registered duplicate on non-canonical cwd path). Separate blocker for the two id:meta projects is the design note at ~/d/science/science/docs/plans/2026-05-22-project-id-uniqueness-and-peer-resolution.md.
