...
analysis, add a recommended next action to run the `science-plan-analysis` skill. Check
...
reflection, and recommend the `science-post-mortem` skill for each:
...
> Reflect on `<failure>` with the `science-post-mortem` skill with input `<failure>` — no lesson has been
...
- The suggested command to run (e.g., the `science-research-topic` skill, the `science-tasks` skill with input `add ...`)
