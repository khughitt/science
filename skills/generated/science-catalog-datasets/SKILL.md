...
2. Read `.ai/templates/dataset.md` first; if not found, read `references/templates/dataset.md`.
...
For brevity, examples write just `science <command>` — **always expand to `uv run science <command>` when executing. See step 8 of the Science Command Preamble above for the fallback.**
